...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      verifiedLeads,
      conversionRate,
      segmentBreakdown: segmentBreakdown.map((s) => ({ segment: s.segment, count: s._count })),
      interestBreakdown: interestBreakdown.map((i) => ({ interest: i.interest, count: i._count })),
      statusBreakdown: statusBreakdown.map((s) => ({ status: s.status, count: s._count })),
      topSources: topSources.map((s) => ({ source: s.source || "Direct", count: s._count })),
      recentLeads,
      leadsByDay,
    });
  } catch (error) {
